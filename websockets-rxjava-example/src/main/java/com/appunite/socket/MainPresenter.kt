...
class MainPresenter(private val socket: Socket, val networkScheduler: Scheduler, val uiScheduler: Scheduler) {
    ...
    private val items: BehaviorSubject<List<AdapterItem>> = BehaviorSubject.create()
    ...
    init {
        ...
        Observable.merge(connectClick.map{ true }, disconnectClick.map{ false })
                .startWith(false)
        ...
        addItem
                .scan(listOf<AdapterItem>()) { adapterItems, adapterItem -> listOf<AdapterItem>().plus(adapterItems).plus(adapterItem) }
                .subscribe(items)
    ...
    private fun mapStringToNewAdapterItem(): Function<String, AdapterItem> = Function { s -> newItem(s, null) }
